...
let print_string_package s = Format.fprintf !formatter_package (Scanf.format_from_string s "")
let print_fmt_package s = Format.fprintf !formatter_package s
...
let documentclass header =
  ...
  if use
  then print_fmt_package "\\NeedsTeXFormat{LaTeX2e}@;\\ProvidesPackage{%s}@;@;" !pfile
...
let command x =
  ...
  in
  !prefix ^ x |> roman |> clear_underscore
